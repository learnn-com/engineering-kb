---
name: capability-write-issue
description: "Creates or updates issues in the adopted PM tool using template-driven formatting. Reads way-of-working for tool choice and type-specific templates for body structure. Invocable independently or composed by /process-refine-story, /process-plan-tasks, /process-plan-initiatives, /process-plan-epics, and /process-plan-stories."
version: 0.4.1
author: Foomakers
---

# /capability-write-issue — PM Tool Issue Writer
...
When composed by `/process-refine-story`:

- **Input**: `/process-refine-story` invokes `/capability-write-issue` with `$type: story`, `$content` containing the refined story data, `$id` when updating an existing story, and `$status: Refined` to transition the board field.
- **Output**: Returns the issue identifier. `/process-refine-story` uses it for linking.

When composed by `/process-plan-tasks`:

- **Input**: `/process-plan-tasks` invokes `/capability-write-issue` with `$type: story`, `$id: [story-id]`, and `$content` containing the Task Breakdown section to append. Tasks are documented inline in the story body — no separate task issues are created.
- **Output**: Returns the story issue identifier. `/process-plan-tasks` confirms the update.

When composed by `/process-plan-initiatives`:

- **Input**: `/process-plan-initiatives` invokes `/capability-write-issue` with `$type: initiative` and `$content` containing the initiative data. Passes `$id` when updating an existing initiative.
- **Output**: Returns the issue identifier. `/process-plan-initiatives` uses it for linking and status tracking.

When composed by `/process-plan-epics`:

- **Input**: `/process-plan-epics` invokes `/capability-write-issue` with `$type: epic`, `$content` containing the epic data, and `$parent` linking to the parent initiative.
- **Output**: Returns the issue identifier. `/process-plan-epics` uses it for linking stories.

When composed by `/process-plan-stories`:

- **Input**: `/process-plan-stories` invokes `/capability-write-issue` with `$type: story`, `$content` containing the story data, and `$parent` linking to the parent epic.
- **Output**: Returns the issue identifier. `/process-plan-stories` uses it for status tracking.
