...
# /process-specify-prd — PRD Creation
...
- **Input**: `/process-bootstrap` detects missing or template PRD and invokes `/process-specify-prd`.
- **Output**: Returns path to the PRD file and approval status.
- `/process-bootstrap` proceeds to next phase only after PRD is approved.
