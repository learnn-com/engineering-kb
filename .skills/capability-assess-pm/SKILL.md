---
name: capability-assess-pm
description: "Assess project management tool using resolution cascade (Argument > Adoption > Assessment). Reads PM tool guidelines, proposes tool choice, writes way-of-working.md PM section, composes /capability-record-decision. Delegates setup to /capability-setup-pm. Idempotent."
version: 0.4.1
author: Foomakers
---

# /capability-assess-pm — PM Tool Assessment

Evaluate and decide on the project management tool. Follows the resolution cascade. After decision, delegates actual tool configuration to `/capability-setup-pm`.
...
| Skill              | Type       | Required                                         |
| ------------------ | ---------- | ------------------------------------------------ |
| `/capability-record-decision` | Capability | Yes — records PM tool decision as ADL            |
| `/capability-setup-pm`        | Capability | Optional — delegates tool configuration if installed |
...
### Step 5: Record Decision (only if /capability-setup-pm not invoked)

1. **Check**: Was `/capability-setup-pm` composed in Step 4? If yes, it already recorded the decision — skip.
2. **Act**: Compose `/capability-record-decision`:
...
├── Decision:  [tool name]
├── Adoption:  [way-of-working.md PM section — written | confirmed | delegated to /capability-setup-pm]
├── Record:    [ADL path — created | exists | delegated to /capability-setup-pm]
...
- **Input**: `/process-bootstrap` invokes during Phase 2 (checklist completion). May pass `$choice`.
- **Output**: Returns decision summary. If `/capability-setup-pm` was composed, returns its output.
...
- If `/capability-setup-pm` not installed, write adoption directly (no tool-specific configuration).
- If `/capability-record-decision` not installed, warn and skip recording.
...
- **Delegation pattern**: /capability-assess-pm decides WHICH tool, /capability-setup-pm configures it. If /capability-setup-pm is installed, it handles both adoption write and decision recording.
