...
- **Input**: `/process-implement` may invoke `/capability-manage-flags` with `$action=create` when a task requires feature-flagged code.
- **Output**: Returns flag creation summary. `/process-implement` continues with the flag available.
