---
name: process-plan-initiatives
description: "Creates strategic initiatives from PRD analysis. Prioritizes by business impact (P0/P1/P2), composes /capability-write-issue with $type: initiative. Idempotent: detects existing initiatives, creates only missing ones."
version: 0.4.1
author: Foomakers
---

# /process-plan-initiatives — Strategic Initiative Creation

Transform Product Requirements Documents into strategic initiatives through PRD analysis, prioritization, and collaborative validation. Composes `/capability-write-issue` for PM tool integration.

## Composed Skills

| Skill          | Type       | Required                                                        |
| -------------- | ---------- | --------------------------------------------------------------- |
| `/capability-write-issue` | Capability | Yes — creates or updates initiative issues in the PM tool       |
...
3. **Act**: If any missing → **HALT**:
   ...
   > Prerequisites incomplete: [list missing]. Complete bootstrap first (`/process-bootstrap`).
...
- If `/capability-write-issue` is not installed, warn and provide formatted initiative content for manual PM tool entry.
...
- After initiative creation, proceed to `/process-plan-epics` for epic breakdown.
