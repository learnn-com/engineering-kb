---
name: process-plan-stories
description: "Breaks epics into user stories using vertical slicing and INVEST validation. Composes /capability-write-issue with $type: story. Idempotent: detects existing stories, creates only missing ones."
version: 0.4.1
author: Foomakers
---

# /process-plan-stories — User Story Breakdown

Transform epics into user stories through vertical slicing, INVEST validation, and collaborative definition. Each story delivers end-to-end user value within a single sprint. Composes `/capability-write-issue` for PM tool integration.

## Composed Skills

| Skill          | Type       | Required                                                  |
| -------------- | ---------- | --------------------------------------------------------- |
| `/capability-write-issue` | Capability | Yes — creates or updates story issues in the PM tool      |
...
5. **Act**: Compose `/capability-write-issue` with:
   - `$type: story`
...
- If `/capability-write-issue` is not installed, warn and provide formatted story content for manual PM tool entry.
...
- Stories at breakdown stage are rough planning units — detailed requirements are added during `/process-refine-story`.
- INVEST validation is mandatory — stories failing INVEST must be reworked before creation.
- After story creation, proceed to `/process-refine-story` for detailed specification.
