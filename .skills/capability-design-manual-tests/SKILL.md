...
├── Categories: [list]
└── Ready for: /capability-execute-manual-tests
```

## Composition Interface

When composed by `/capability-execute-manual-tests` (Step 1 gate):

- **Input**: `/capability-execute-manual-tests` detects no suite → suggests running `/capability-design-manual-tests`.
- **Output**: Returns the suite directory path. `/capability-execute-manual-tests` can then re-invoke with `$suite` pointing to the generated directory.

When invoked **independently**:

- Generates the full suite and presents the summary.
- User can then invoke `/capability-execute-manual-tests` to run it.
