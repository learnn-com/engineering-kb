---
name: process-plan-epics
description: "Breaks initiatives into epics through structured analysis and collaborative validation. Composes /capability-write-issue with $type: epic. Idempotent: detects existing epics, creates only missing ones. Includes Epic 0 assessment for new projects."
version: 0.4.1
author: Foomakers
---

# /process-plan-epics — Epic Breakdown

Transform strategic initiatives into comprehensive epic breakdowns. Each epic delivers end-to-end user value in 2-4 sprints. Composes `/capability-write-issue` for PM tool integration.

## Composed Skills

| Skill          | Type       | Required                                                  |
| -------------- | ---------- | --------------------------------------------------------- |
| `/capability-write-issue` | Capability | Yes — creates or updates epic issues in the PM tool       |
...
1. **Act**: Analyze initiative components:
   ...
   - Architecture alignment for service boundaries.
...
- If `/capability-write-issue` is not installed, warn and provide formatted epic content for manual PM tool entry.
...
- After epic creation, proceed to `/process-plan-stories` for user story breakdown.
