...
4. **Verify**: Record quality gate results. Failures become review findings (do not HALT — /process-review reports them).
...
2. For each non-conformity:
   - **Tech-stack**: compose `/capability-assess-stack` → developer approves (add to stack) or rejects (CHANGES-REQUESTED).
...
3. **Act**: Compose `/capability-assess-debt` with `$scope = all`.
...
1. **Act**: Compile all findings into a review report following the [code-review-template.md](../../.pair/knowledge/guidelines/collaboration/templates/code-review-template.md):
   ...
   - **Tech Debt**: items flagged by /capability-assess-debt
...
Re-invoking `/process-review` on a partially reviewed PR is safe:
...
3. **Skill compositions**: /capability-verify-quality, /capability-verify-done results cached in session. Not re-run if already passing on current commit.
