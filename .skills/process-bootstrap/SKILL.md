...
description: "Orchestrates full project setup: PRD verification, project categorization, checklist completion, standards generation, quality gate setup, and PM tool configuration. Composes /process-specify-prd, /capability-setup-pm, /capability-record-decision, and assess-* (optional). Idempotent — detects completed phases and resumes."
version: 0.4.1
author: Foomakers
---

# /process-bootstrap — Project Bootstrap
...
| Skill                   | Type       | Required                                                                      |
| ----------------------- | ---------- | ----------------------------------------------------------------------------- |
| `/process-specify-prd`          | Process    | Yes — invoked if PRD is missing or template                                   |
| `/capability-setup-pm`             | Capability | Yes — invoked in finalization phase for PM tool configuration                 |
| `/capability-record-decision`      | Capability | Yes — invoked for each bootstrap decision (categorization, tech choices, etc) |
| `/capability-assess-architecture`  | Capability | Optional — architecture pattern assessment. Graceful degradation if absent.   |
| `/capability-assess-stack`         | Capability | Optional — tech stack assessment (core sections). Graceful degradation if absent. |
| `/capability-assess-testing`       | Capability | Optional — testing strategy assessment. Graceful degradation if absent.       |
| `/capability-assess-infrastructure`| Capability | Optional — infrastructure assessment. Graceful degradation if absent.         |
| `/capability-assess-observability` | Capability | Optional — observability assessment. Graceful degradation if absent.          |
| `/capability-assess-methodology`   | Capability | Optional — methodology assessment. Graceful degradation if absent.            |
| `/capability-assess-pm`            | Capability | Optional — PM tool assessment (delegates to /capability-setup-pm). Graceful degradation if absent. |
| `/capability-assess-ai`            | Capability | Optional — AI development tools assessment. Graceful degradation if absent.   |
...
3. **Act**: On confirmation, compose `/capability-record-decision`:
...
2. **Act** (installed): Compose assess-\* skills in recommended sequence. Each skill checks its own adoption file first — already-decided domains are skipped automatically (resolution cascade).
   ...
   **Recommended sequence** (respects adoption file dependencies):
   1. `/capability-assess-architecture` → writes `architecture.md` (needed by stack and infrastructure)
   2. `/capability-assess-stack` → writes core sections of `tech-stack.md` (needed by testing and AI)
   3. `/capability-assess-testing` → writes testing section of `tech-stack.md`
   4. `/capability-assess-ai` → writes AI section of `tech-stack.md`
   5. `/capability-assess-infrastructure` → writes `infrastructure.md` (needed by observability)
   6. `/capability-assess-observability` → writes observability section of `infrastructure.md`
   7. `/capability-assess-methodology` → writes methodology section of `way-of-working.md`
   8. `/capability-assess-pm` → writes PM section of `way-of-working.md` (delegates to `/capability-setup-pm`)

   **Section ownership** (prevents parallel write conflicts):

   | Adoption File        | Section            | Owner Skill            |
   | -------------------- | ------------------ | ---------------------- |
   | `architecture.md`    | Full file          | `/capability-assess-architecture` |
   | `tech-stack.md`      | Core sections      | `/capability-assess-stack`        |
   | `tech-stack.md`      | Testing section    | `/capability-assess-testing`      |
   | `tech-stack.md`      | AI section         | `/capability-assess-ai`           |
   | `infrastructure.md`  | Core sections      | `/capability-assess-infrastructure` |
   | `infrastructure.md`  | Observability      | `/capability-assess-observability`|
   | `way-of-working.md`  | Methodology        | `/capability-assess-methodology`  |
   | `way-of-working.md`  | PM tool            | `/capability-assess-pm`           |
   | `way-of-working.md`  | Quality gates      | `/process-bootstrap` (Step 3.2)|
...
- Record each significant decision via `/capability-record-decision` (`non-architectural` → ADL, `architectural` → ADR)
...
5. **Act**: Record quality gate decisions via `/capability-record-decision`:
   - `$type`: `non-architectural`
...
3. **Act**: Compose `/capability-setup-pm`. The skill handles tool selection, configuration, and ADL recording.
...
```text
BOOTSTRAP COMPLETE:
├── PRD:             [verified | created via /process-specify-prd]
...
├── PM Tool:         [configured via /capability-setup-pm | already configured]
...
- **PRD missing or template and /process-specify-prd fails** (Phase 0) — cannot bootstrap without product context
...
Re-invoking `/process-bootstrap` on a partially completed project is safe and expected:
...
- **/setup-pm not installed**: Skip PM configuration in Phase 4. Warn: "PM tool not configured — /capability-setup-pm not installed."
- **/record-decision not installed**: Skip decision recording. Warn: "Decisions not recorded — /capability-record-decision not installed. Document decisions manually in adoption files."
...
- All decisions during bootstrap are recorded via `/capability-record-decision`. Non-architectural → ADL. Architectural → ADR.
