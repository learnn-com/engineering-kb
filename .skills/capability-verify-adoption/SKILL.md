---
name: capability-verify-adoption
description: "Checks code and configuration against project adoption files for a given scope. Detection-only: returns conformity/non-conformity list per area without resolving issues. Caller decides resolution. Invocable independently or composed by /process-review and /process-implement."
version: 0.4.1
author: Foomakers
---

# /capability-verify-adoption — Adoption Compliance Checker
...
1. [area] — [description] ([file:line])
   Resolution: [/assess-stack | /capability-record-decision | developer action]
2. ...
...
| ---------------- | ----------------------- | ---------------------------------------- |
| Tech Stack       | `/capability-assess-stack`         | Stack changes go through tech assessment |
| Architecture     | `/capability-record-decision`      | Architectural gaps need an ADR           |
...
- **Input**: `/process-review` invokes `/capability-verify-adoption` with `$scope=all` during the adoption compliance phase.
- **Output**: Returns the compliance report. /process-review incorporates findings into review output.
...
- **Input**: `/process-implement` may invoke `/capability-verify-adoption` with a targeted `$scope` (e.g., `tech-stack`) before commit.
- **Output**: Returns the compliance report. /process-implement resolves non-conformities via /capability-assess-stack or /capability-record-decision.
  - Unresolved non-conformities do not HALT — /process-implement warns and continues.
...
- If `$scope=all` and no adoption files exist at all, report: "No adoption files found — run `/process-bootstrap` to establish project standards."
