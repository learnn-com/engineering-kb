---
name: capability-assess-code-quality
description: "Assesses code quality using resolution cascade (Argument > Adoption > Assessment). Applies quality score formula from code-metrics guidelines. Produces quality report with complexity, size, coverage, and maintainability metrics. Idempotent: detects existing report, checks staleness. Invocable independently or composed by /process-review."
version: 0.4.1
author: Foomakers
---
...
- **Input**: /process-review may invoke `/capability-assess-code-quality` during the technical review phase.
- **Output**: Returns the quality report. /process-review incorporates metrics and hotspots into review findings.
