...
- **Input**: The composing skill invokes `/capability-verify-quality` after implementation or before commit.
- **Output**: Returns PASS or FAIL with details. The composing skill decides what to do:
  - `/process-implement`: HALT on FAIL — developer must fix before commit.
  - `/process-review`: Report FAIL as review finding — does not block review completion.
