---
name: capability-execute-manual-tests
description: "Executes a project's manual test suite against released artifacts, producing a structured report. Resolves test variables, iterates through critical paths, records PASS/FAIL per test case, and generates the report from the manual-test-report template. Invocable independently or composed by /process-review (post-merge validation)."
version: 0.4.1
author: Foomakers
---
...
2. **Skip**: If no suite found and no `$suite` argument → **HALT**: "No manual test suite found. Run `/capability-design-manual-tests` to generate one from your project's artifacts, then re-invoke `/capability-execute-manual-tests`."
3. **Check**: If `$suite` directory exists but contains zero `CP*.md` files → **HALT**: "Suite directory exists but contains no critical path files. Run `/capability-design-manual-tests --output {$suite}` to populate it."
...
When composed by `/process-review` (Phase 6, post-merge):

- **Input**: /process-review invokes `/capability-execute-manual-tests` after merge as optional post-release validation.
...
- **Scope**: /process-review passes `$scope = P0` (blockers only) for fast post-merge validation. Full suite runs standalone.

When composed by `/capability-verify-done` (Step 5.5, optional):

- **Input**: /capability-verify-done checks if a manual test report exists for the current version with PASS result.
- **Output**: Returns PASS (report exists and passing) or SKIPPED (no suite or no report).
- **Note**: /capability-verify-done does NOT invoke /capability-execute-manual-tests — it only checks for an existing report.
...
- **No test suite found**: HALT with guidance to run `/capability-design-manual-tests` first.
